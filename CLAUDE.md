...
The same rule now covers **which activities a day asks for**, because those are generated from `journeys.json` rather than written down. Naming `day-1-walk-1km` in a test broke twenty-four of them the moment the movements changed, and broke them as silent no-ops — `toggleActivityCompletion` ignores an activity that is not in the day's plan, so the test failed on a count rather than on anything that named the cause. `src/test/journey-baseline.ts` exports `activityIdForDay` and `activityIdsForDay`, which read the ids off the generated plan; use those rather than literals whenever a test needs an activity that actually exists.
...
- `features/journeys` is the one domain whose data is **generated rather than stored**, and the one with a three-part model. A **journey** is a definition — what a day looks like and how many days there are — and deliberately carries no start date; an **enrollment** is someone running that journey from a given date; **completions** hang off the enrollment. That split is what lets the same journey be started twice, or by different people, without the runs colliding, and it is why every selector takes an `EnrolledJourney` pair rather than a journey.
  ...
  The day plans are not fixtures. `journey-plan.ts` derives them from the definition and the day index as a pure function, and **movements and books are dealt the same way**: a seeded Fisher-Yates permutation of the track's list, reshuffled per cycle, so every entry comes up exactly once per pass while the order inside a pass looks arbitrary. A fixed `physicalRotation` used to pick the movements and was replaced because it could not do that — seven entries over a hundred days is the same seven combinations fourteen times, however many movements the journey defines. That determinism is load-bearing, not decorative: a completion row points at an activity id like `day-4-growth-reading`, so a plan that changed between renders would silently re-point it. `journey-store.ts` is therefore the only place that can tell a real activity id from an invented one, and it checks the generated plan before writing.

  Physical work has **two tracks and a progression**, and both exist for the same reason the hundred days are meant to be survivable. A day draws one movement from `main` and one from `easy`, so a tired day is still a day of movement rather than a skipped one. Each movement also carries a `focus` — what it actually taxes — because dealing every movement once per pass keeps the _movements_ varied while saying nothing about the muscles: a plain shuffle left twenty-seven of the ninety-nine adjacent pairs sharing a focus, including three pushing days running. `spaceOutPhysicalFocus` re-orders each shuffled pass greedily so the next movement's focus differs from the one just used, taking the first remaining only when everything left taxes the same thing; passes are built from the first rather than jumped to, so the boundary between two passes is spaced as well. It reorders and never adds or drops, so the even load the permutation exists to give survives. Two adjacent clashes remain on `main` over the hundred days, and `journey-plan.test.ts` holds a budget rather than zero, because a pass whose tail all taxes one thing cannot be solved. Each movement then carries ordered `levels` spread evenly across the journey — `selectPhysicalLevelIndex` reads the level off the day index, never off the draw, so a movement asks the same thing whenever it lands on a given day. A level may raise the reps or raise the difficulty at the same reps (30 squats → 30 goblet squats → 30 slow goblet squats), which is the honest way to keep progressing once more reps stop meaning more. **The completion id embeds the movement id, not the level**, so a movement that levels up on day twenty-six does not orphan the ticks written against it before.

  Every part of a journey is optional except that it must schedule _something_ — `journeySchema` refuses a definition with no physical work, no books and no reflection, because otherwise every one of its days would be vacuously complete. The `deep-work-reset` fixture exists to keep that generality honest: a different length, no physical work at all and a single reading track, so a custom journey cannot be the first thing to exercise those paths. `journey-api.ts` is the third unusual piece — see above.
