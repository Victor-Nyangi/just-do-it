...
- **`pnpm test` runs vitest, and every route now has a rendering test.** All six domains have logic tests: habit selectors, schemas and store; task selectors; goal selectors and store; book store; list selectors and store; the quick-add parser and its `toQuickAddTaskInput` adapter; `features/calendar`'s date-mapping selectors; `features/challenge`'s schemas, plan builder, selectors, store and `challenge-api`; and `features/command-palette`'s pure `buildCommands`. On top of those, all twelve real routes render under test — `/today`, `/tasks`, `/habits`, `/habits/:habitId`, `/lists`, `/lists/:listId`, `/books`, `/goals`, `/calendar`, `/challenge`, `/challenge/books`, `/challenge/streak` — alongside component-level suites for `QuickAddField`, `CommandPalette`, the `useGlobalShortcuts` hook, `AppLayout`'s mobile drawer, and the `Command` primitive itself (581 tests, 36 files). The primitive is covered from `apps/just-do-it/src/test/ui-command.test.tsx` because `packages/ui` has no vitest of its own; a new primitive gets its test there. `/settings` is still `<PlaceholderPage>` and has none. The `/tasks` suite also exercises `TaskFiltersPanel`, `TaskForm` and `TaskList`, which the route delegates to; `HabitDayGrid` is reached only through the routes that render it, for the reason in the plan's debt list. A passing `build` is still not verification of behavior beyond types.
...
One more trap for date-driven routes: the fixtures only span **May–September 2026** — except `features/challenge`, whose window is **11 September – 19 December 2026**, so a clock pinned for a challenge route is pinned outside every other domain's data and vice versa. A test pinned
...
`features/challenge/challenge-api.ts` is the one apparent exception, and is not one: the challenge was specified as three HTTP endpoints (`GET /api/challenge/day/[dayIndex]`, `POST /api/challenge/day/[dayIndex]/complete`, `GET /api/challenge/stats`), and that module keeps their contracts — addressing by day index, the same response shapes, toggle semantics on complete, `null` standing in for a 404 — while running in-process against the store. It adds no fetch layer and no persistence. Moving the challenge behind a real API means rewriting those three function bodies and touching nothing that calls them. The planned `challenge_completions` row is `(user_id, day_index, activity_id, completed_at)`; the client type omits `user_id` on purpose, because that column belongs to the authenticated session rather than to anything the client should be asserting.
...
- `features/challenge` is the one domain whose data is **generated rather than stored**. Its fixtures hold the challenge definition, the book list and the completions — but not the hundred day plans, which `challenge-plan.ts` derives from the day index as a pure function: the physical rotation by `(dayIndex - 1) % 7`, and each track's book by a seeded Fisher-Yates permutation of that track's list, reshuffled per cycle so every book still comes up exactly once per pass. That determinism is load-bearing, not decorative: a completion row points at an activity id like `day-4-growth-reading`, so a plan that changed between renders would silently re-point it. `challenge-store.ts` is therefore the only place that can tell a real activity id from an invented one, and it checks the generated plan before writing. `challenge-api.ts` is the third unusual piece — see below.
...
Routing is flat in `App.tsx` under a single `AppLayout` outlet, with nested dynamic routes for habit and list detail (`/habits/:habitId`, `/lists/:listId`) and three flat challenge routes (`/challenge`, `/challenge/books`, `/challenge/streak`). `/settings` is still `<PlaceholderPage>`; `/habits` is not. Unknown paths redirect to `/today`.
