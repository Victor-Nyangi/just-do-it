...
- **`pnpm test` runs vitest, but coverage is narrow.** It covers habit selectors, schemas, and the store, plus one lists selector. No route or component has a test. A passing `build` is still not verification of behavior beyond types — and a passing `test` only verifies the pieces listed above, not the app as a whole.
