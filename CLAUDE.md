...
- **`pnpm test` runs vitest, and every route now has a rendering test.** All six domains have logic tests: habit selectors, schemas and store; task selectors; goal selectors and store; book store; list selectors and store; the quick-add parser and its `toQuickAddTaskInput` adapter; `features/calendar`'s date-mapping selectors; `features/challenge`'s schemas, plan builder, selectors, store and `challenge-api`; and `features/command-palette`'s pure `buildCommands`. On top of those, all twelve real routes render under test — `/today`, `/tasks`, `/habits`, `/habits/:habitId`, `/lists`, `/lists/:listId`, `/books`, `/goals`, `/calendar`, `/challenge`, `/challenge/books`, `/challenge/streak` — alongside component-level suites for `QuickAddField`, `CommandPalette`, the `useGlobalShortcuts` hook, and the `Command` primitive itself (574 tests, 35 files). The primitive is covered from `apps/just-do-it/src/test/ui-command.test.tsx` because `packages/ui` has no vitest of its own; a new primitive gets its test there. `/settings` is still `<PlaceholderPage>` and has none. The `/tasks` suite also exercises `TaskFiltersPanel`, `TaskForm` and `TaskList`, which the route delegates to; `HabitDayGrid` is reached only through the routes that render it, for the reason in the plan's debt list. A passing `build` is still not verification of behavior beyond types.
