...
- **`pnpm test` runs vitest, and coverage is selector- and store-deep but UI-blind.** All five domains now have tests: habit selectors, schemas and store; task selectors; goal selectors and store; book store; list selectors and store. **No route or component has a test**, so nothing renders under test and `calendar-page.tsx`'s date mapping is still unverified. A passing `build` is not verification of behavior beyond types, and a passing `test` says nothing about the UI.
