...
describe('habit store persistence', () => {
  ...
  it('does not clobber in-memory state with the fixtures when storage is empty', async () => {
    // The old version of this test called reload(), which itself begins with
    // setState({ habits: getInitialHabits(), completions: getInitialHabitCompletions() })
    // -- so both toHaveLength(fixture.length) checks were satisfied by the
    // harness's own setState, regardless of whether rehydrate()/merge did
    // anything at all.
    //
    // To discriminate, put the store into a state the fixtures could never
    // produce (zero habits, zero completions), make sure storage is genuinely
    // absent, and rehydrate directly (not via reload(), which would
    // immediately re-seed both fixtures and defeat the point).
    //
    // The assertion is that both slices STAY at zero, not that the fixtures
    // reappear: createValidatedMerge's fallback returns `current` verbatim
    // (see store-persistence.ts), and it has no notion of "the fixture" at
    // all -- current only happens to BE the fixtures in production because
    ...
    useHabitStore.setState({ habits: [], completions: [] }); // distinguishable; write-through fires
  ...
  it('rejects a persisted habit that fails its schema, and warns naming the store', async () => {
    ...
    expect(warnSpy.mock.calls[0][0]).toContain('habits');
  });
