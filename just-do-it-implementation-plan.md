...
| Tests           | vitest                                                    | 267 tests across 13 files, all five domains — selectors, stores, schemas, the quick-add parser and the calendar mapping — plus a rendering test for the calendar route. Eight routes still untested |
...
- **Test coverage is deep on logic, and now spans five routes and one component.** 342 vitest tests
  across 18 files cover every domain's selectors and stores, the Zod round-trip on mutation, the
  ...
  calendar route, 16 for `/tasks`, 14 for `/today`, 13 each for `/habits` and `/habits/:habitId`,
  13 for `QuickAddField`, and 4 guarding the test harness itself. The `/tasks` suite reaches
  `TaskFiltersPanel`, `TaskForm` and `TaskList` through the route. The other four routes (`lists`,
...
- **Untested calendar-route behaviour, reachable now that the harness exists.** Indicator chips on
  ...
  branch that built the rendering layer, and all are one short test each.
...
- [ ] React Testing Library — installed and proven on `/calendar` (including one block pinned to a
      ...
      `TaskList` through the route), and on `QuickAddField` itself, the one component that had
      shipped verified by eye alone; `lists`, `list-detail`, `books` and `goals` are still
